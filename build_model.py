...
class CIM(nn.Module):
    ...
    def refine_word_pre(self, model_pre, haf_pre, k, beta):
        model_pre_minmax = self.minmax_oprtator(model_pre)
        model_pre = torch.max(model_pre_minmax, dim=1)[0]
        
        haf_pre = torch.sum(F.one_hot(haf_pre.long(), self.num_words), dim=1).float()
        
        predicts_refine = (1.-beta) * self.softmax(model_pre) + self.softmax(haf_pre) * beta
...
class FeatureFusion(nn.Module):
    ...
    def forward(self, feature_global, feature_cim):
        feature_global = self.linearg2g(feature_global)
        feature_cim = self.linearl2g(feature_cim)
        feature_fusion = ((1.-self.beta) * feature_global) + (feature_cim * self.beta)
...
def build(CUDA, opt):
    ...
    if opt.method=='global':
        ...
        model = ARL(CUDA, encoder_v, dim_feat_v, opt.num_words)
    elif opt.method=='haf':
        encoder_v = select_visual_network(opt.net_v, opt.size_img, opt, return_fm=True)
        model_arl = ARL(CUDA, encoder_v, dim_feat_v, opt.num_words)
        # load model in task2
        model_arl = get_updateModel(model_arl, opt.path_arl)
        model = HAF(model_arl, dim_feat_v, opt.num_cls, opt.num_words, [opt.top_cls, opt.top_seq, opt.top_pos, opt.topk], opt.cluster_weights, opt.cluster_label_indicators, opt.valid_cluster_class, opt.feature_max, opt.feature_min)
    elif opt.method=='cim':
        model = CIM(dim_feat_v, opt.num_cls, opt.num_words, opt.topk, opt.beta_know, opt.beta_relation)
