...
def test_fusion(data_loader, model, opt):
    ...
    for batch_idx, (data, label) in enumerate(data_loader):        
        ...
        [feature_global, feature_cim] = data
        
        label = label.long().cuda()
        feature_global = feature_global.cuda()
        feature_cim = feature_cim.cuda()
        
        _, pre_fusion = model(feature_global, feature_cim)
...
def generate_feature_cim(data_loader, model, opt):
    model.eval()
    with torch.no_grad():
        for batch_idx, (data, label) in enumerate(data_loader):
            ...
            feature_cim, output_cim = model(word_predicts, feature_v, decision_words)
            
            if batch_idx==0:
                feature_cim_all = torch.zeros((0, feature_cim.shape[1]))
                output_cim_all = torch.zeros((0, opt.num_cls))
                label_all = torch.zeros(0)
            feature_cim_all = torch.cat((feature_cim_all, feature_cim.cpu()), 0)
            output_cim_all = torch.cat((output_cim_all, output_cim.cpu()), 0)
            label_all = torch.cat((label_all, label.cpu()), 0)

    print('CIM features generated.')

    return feature_cim_all.detach().cpu(), output_cim_all.detach().cpu(), label_all.detach().cpu()
